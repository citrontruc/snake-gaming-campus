...
public class DirectionBlockTests
{
    ...
    public void DirectionBlock_ShouldBePlaced_OnGrid()
    {
        ...
        Assert.AreEqual(
            pos,
            block.GetPosition(),
            "DirectionBlock should store the correct position"
        );
        Assert.AreEqual(
            CellCoordinates.down,
            block.GetDirection(),
            "DirectionBlock should store the correct direction"
        );
        Assert.AreEqual(
            block.GetID(),
            _grid.Cells[pos.X, pos.Y],
            "Grid should mark the block's cell as occupied"
        );
    }
    ...
    public void DirectionBlock_ShouldDisable_WhenCollided()
    {
        ...
        Assert.AreEqual(
            Entity.EntityState.disabled,
            block.GetState(),
            "Block should be disabled after collision"
        );
        Assert.AreEqual(0, _grid.Cells[4, 4], "Block's grid cell should be freed");
        Assert.IsTrue(
            _playerHandler.GetQueue().Contains(block),
            "Block should be added back to PlayerHandler queue"
        );
    }
    ...
    public void DirectionBlock_Reset_ShouldDisableActiveBlock()
    {
        ...
        Assert.AreEqual(
            Entity.EntityState.disabled,
            block.GetState(),
            "Reset should disable the block"
        );
        Assert.AreEqual(0, _grid.Cells[1, 1], "Reset should free the grid cell");
        Assert.IsTrue(
            _playerHandler.GetQueue().Contains(block),
            "Reset should add block back to PlayerHandler queue"
        );
    }
}
